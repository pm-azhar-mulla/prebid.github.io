...
### Prebid Server Note:
Before configuring the GroupM adapter as S2S, you must reach out to the GroupM team for approval and setup steps.

### Prebid 1.0 Upgrade Note:
If you upgrading from a Prebid version prior to 1.0, please reach out to your GroupM Customer Success Manager prior to your upgrade.  Publisher accounts need new settings to function correctly with the GroupM Prebid 1.0 adapter and your Customer Success Manager will ensure your account is setup correctly.
...
GroupM recommends the UserSync configuration below.  Without it, the GroupM adapter will not able to perform user syncs, which lowers match rate and reduces monetization.
...
Only With Size is supported.  To support multi-size ad units, list the GroupM bidder entry multiple times, once for each size.  Mutli-sized ad units are not supported when using Prebid Server.

### video parameters
The GroupM adapter supports video as of Prebid v1.16.0
...
var adUnits = [
{
    ...
    bids: [{
      ...
      params: {
        ...
        adSlot: 'groupm_test2@1x1',       // required
      }
    }]
}];
```
...
For Video ads, prebid cache needs to be enabled for GroupM adapter.
```
pbjs.setConfig({
    cache: {
